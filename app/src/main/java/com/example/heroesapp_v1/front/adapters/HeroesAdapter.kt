...
class HeroesAdapter(private val heroClickListener: HeroClickListener) :  RecyclerView.Adapter<ViewHolder>(){
    ...
    //private var favoriteList : List<Boolean> = emptyList()
    ...
    /**fun setFavorites(favList: List<Boolean>) {
    ...
    fun setData (heroList: List<Hero>) {
        // Check if no more data from API
    ...
    fun updateFavoriteHero(hero: Hero, heroPosition: Int) {
        itemsToDisplay[heroPosition].isFavorite = hero.isFavorite
        notifyDataSetChanged()
    }
        ...
        if (heroList.size == itemsToDisplay.size) {
            notifyDataSetChanged()
            return
        }*/
...
class ViewHolder constructor( itemView: View, private val heroClickListener: HeroClickListener): RecyclerView.ViewHolder(itemView) {
    ...
    fun bindViewsToViewHolder(hero: Hero) {
        ...
        itemView.setOnClickListener {
            heroClickListener.onHeroClicked(adapterPosition, hero.isFavorite)
        }
